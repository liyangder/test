...
class Page{
    ...
    public function __construct($totalRows, $listRows=20, $parameter = array()) {
        ...
        // 自定义的替换数据
        // 每页的第一条
        $this->page_first = $this->firstRow + 1;
        // 每页的最后一条
        $this->page_last = $this->firstRow + $this->listRows;
        // 考虑最后一个未满的情况
        $this->page_last>$this->totalRows && $this->page_last = $this->totalRows;
    }
    ...
    public function show() {
        ...
        $page_str = str_replace(
            array('%HEADER%', '%NOW_PAGE%', '%UP_PAGE%', '%DOWN_PAGE%', '%FIRST%', '%LINK_PAGE%', '%END%', '%TOTAL_ROW%', '%TOTAL_PAGE%', '%PAGE_FIRST%', '%PAGE_LAST%'),
            array($this->config['header'], $this->nowPage, $up_page, $down_page, $the_first, $link_page, $the_end, $this->totalRows, $this->totalPages, $this->page_first, $this->page_last),
            $this->config['theme']);
