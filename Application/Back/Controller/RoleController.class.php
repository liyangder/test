    ...
    public function addAction()
    {
        if (IS_POST) {
            ...
            if ($model->create()) {
                ...
                $id = $model->add();
                if ($id) {
                    // 当前角色添加成功, 判断是否为超级管理员, 如果是, 消除其他的超级管理员
                    if (I('post.is_super') == '1') {
                        $cond = [
                            'is_super'=> 1,
                            'id' => ['neq', $id]
                        ];
                        $model->where($cond)->save(['is_super'=>0]);
                    }
...
//        完成查询
    ...
    public function multiAction()
    {
        ...
        switch($operate) {
            case 'delete':
                M('Role')->where(['id'=>['in', $pk_list]])->delete();
                $this->redirect('list');
                break;
        }
    }
...
//            更新当前权限
//            $id ,角色ID
            $action_id_list = I('post.action_id', []);
            ...
            // 删除旧的关联
            M('RoleAction')->where(['role_id'=>$id])->delete();
            // 插入新关联
            M('RoleAction')->addAll($rows);
            // 重定向
            $this->redirect('grant', ['id'=>$id]);

        } else {
//            展示当前权限
            //        获取全部权限
            $model_action = D('Action');
            $action_list =  $model_action->getNested();
            $this->assign('action_list', $action_list);

//            已选择权限
            $this->assign('checked_action_list', M('RoleAction')->where(['role_id'=>$id])->getField('action_id', true));
